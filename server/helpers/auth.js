...
const getOrAddUser = function(accessToken, refreshToken, profile, callback) {
  ...
  db.any('INSERT INTO users AS u (id, username, email, created_ga, updated_ga, signed_up, avatar_url, followers, following) ' +
    ...
    .then(() => {
      ...
      // we are sending false as the last argument to run the requests independently - if any of
      // them fail, the rest will still run
      massiveFetch(id, username, accessToken, profile, false);
    })
    .catch((error) => {
